...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
	...
	if (client->getCharacterCount(zoneServer.get()->getGalaxyID()) >= 8) {
		ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are limited to 8 characters per account.", 0x0);
				...
				if (accountPermissionLevel < 9) {
					try {
						...
						if (res != NULL && res->next()) {
							...
							Time timeVal(sec);
							// woohoori 20191014 adjusted character creation down from 1 hour (3600000) to 1 min
